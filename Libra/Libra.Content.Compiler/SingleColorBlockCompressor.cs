...
{
    public static class SingleColorBlockCompressor
    {
        public static BC2BitmapContent Compress(PixelBitmapContent<Color> source, Color color0)
        {
            ...
            var result = new BC2BitmapContent(source.Width, source.Height);
            result.SetPixelData(outputData);

            return result;
        }

        static void CompressDxt3Block(
            PixelBitmapContent<Color> source, int blockX, int blockY, ushort color0, byte[] outputData, int outputIndex)
        {
            long alphaBits = 0;
            int rgbBits = 0;
            int pixelCount = 0;
